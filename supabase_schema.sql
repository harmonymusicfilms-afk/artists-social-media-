...
create table public.studios (
  ...
  owner_id uuid references public.profiles(id),
  name text not null,
  type text not null, -- 'Recording', 'Video', etc.
  location text,
  address text,
  cover_photo text,
  gallery text[],
  description text,
  equipment jsonb default '[]'::jsonb, -- Array of {name, brand}
  amenities text[],
  pricing jsonb default '{}'::jsonb, -- {hourly, daily, monthly}
...
-- 8. NGO/SHG GROUPS TABLE
create table public.ngo_shg_groups (
  id bigint generated by default as identity primary key,
  created_at timestamp with time zone default timezone('utc'::text, now()),
  owner_id uuid references public.profiles(id),
  name text not null,
  type text not null, -- 'NGO' or 'SHG'
  focus_area text,
  location text,
...
alter table public.ngo_shg_groups enable row level security;

create policy "Groups are viewable by everyone."
  on ngo_shg_groups for select
  using ( true );

create policy "Authenticated users can insert groups."
  on ngo_shg_groups for insert
  with check ( auth.role() = 'authenticated' );

-- 9. MUSIC PRODUCTS TABLE
...
alter table public.music_products enable row level security;

create policy "Products are viewable by everyone."
  on music_products for select
  using ( true );

create policy "Authenticated users can insert products."
  on music_products for insert
  with check ( auth.role() = 'authenticated' );
